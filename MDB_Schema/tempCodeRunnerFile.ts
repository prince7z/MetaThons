...
interface IUser {
    ...
    notice: { _id: Types.ObjectId; status: string }[];
    applied: { _id: Types.ObjectId; status: string }[];
    notifications: {
        type: string;
        message: string;
        from: Types.ObjectId;
    ...
    try {
        ...
        for (const wantedEntry of wanted) {
            ...
            for (const applicant of applicants) {
                // Add to applied array as object with status
                applicant.applied.push({ _id: wantedEntry._id, status: faker.helpers.arrayElement(['pending', 'accepted', 'rejected']) });
                // Add to notice array as object with status (simulate some users being noticed for this wanted)
                if (faker.datatype.boolean()) {
                    applicant.notice.push({ _id: wantedEntry._id, status: faker.helpers.arrayElement(['pending', 'accepted', 'rejected']) });
                }
                // Add notification for application
